...
# Project: Unreliable heterogeneity
...
## Table 1
### Variance in reliability leads to positive bias in heterogeneity estimates
...
dat %>% knitr::kable(booktabs = TRUE, escape = FALSE, digits = 2,
                    align=rep('c', 5),
...
```{r, alternative negative bias table}
#Alternative table
rel <- c("$\\sqrt{R_{xx'}}\\times\\sqrt{R_{yy'}}$ = 1",
         "$\\sqrt{R_{xx'}}\\times\\sqrt{R_{yy'}}$ = 0.8",
         "$\\sqrt{R_{xx'}}\\times\\sqrt{R_{yy'}}$ = 0.6")
...
saveRDS(list(positve_bias_table = positive_bias_table,
